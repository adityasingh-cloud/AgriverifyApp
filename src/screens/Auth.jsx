...
export function Auth() {
  const { login } = useAuth();
  const [step, setStep] = useState(1); // 1: Login, 2: Onboarding
  ...
  return (
    <div className="h-screen w-screen bg-agri-bg flex flex-col justify-center items-center p-6 relative overflow-hidden">
      ...
      >
        ...
        {step === 1 && (
          <div className="text-center">
            ...
            >
              ...
              <span className="font-semibold text-white">Continue with Phone</span>
            </button>
        ...
        {step === 2 && (
          <div>
            <h2 className="text-2xl font-display font-black text-white mb-1">Complete Profile</h2>
            ...
            <form onSubmit={handleComplete} className="space-y-4">
              <input 
                ...
                className="w-full bg-agri-card border border-agri-border rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:border-agri-green outline-none transition-colors"
              />
              <div className="flex gap-4">
                <input 
                  type="date" required 
                  value={formData.dob} onChange={e => setFormData(p => ({...p, dob: e.target.value}))}
                ...
                />
                <select 
                  value={formData.gender} onChange={e => setFormData(p => ({...p, gender: e.target.value}))}
                  className="flex-1 bg-agri-card border border-agri-border rounded-xl px-4 py-3 text-white focus:border-agri-green outline-none appearance-none"
                >
                  <option>Male</option>
              ...
              <button 
                type="submit"
                className="w-full mt-6 bg-gradient-to-r from-agri-green to-agri-green-dim py-4 rounded-xl font-bold text-white shadow-[0_8px_24px_rgba(34,197,94,0.25)] flex justify-center items-center gap-2 hover:scale-[0.98] transition-transform"
              >
                Start Using AgriVerify <ArrowRight size={18} />
              </button>
            </form>
          </div>
        )}
