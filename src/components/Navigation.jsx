...
export function Navigation({ currentTab, setCurrentTab, onCameraClick }) {
  const { t } = useLang();
  return (
    <div className="fixed bottom-0 w-full bg-white border-t-2 border-black/5 px-6 pt-4 pb-[calc(1rem+env(safe-area-inset-bottom))] flex justify-between items-center z-40 shadow-[0_-10px_40px_rgba(0,0,0,0.03)]">
      ...
      <div className="relative -top-8">
        <button 
          ...
          className="w-18 h-18 w-[72px] h-[72px] rounded-[20px] bg-black flex items-center justify-center shadow-2xl shadow-black/30 hover:scale-95 transition-transform border-4 border-white"
        >
...
function NavItem({ icon, label, active, onClick }) {
  return (
    <button 
      onClick={onClick} 
      className={`flex flex-col items-center gap-1.5 transition-all ${active ? 'text-black scale-110' : 'text-gray-300'}`}
    >
